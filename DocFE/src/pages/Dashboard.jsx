            ...
            <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, scale: 0.95 }} className="mb-8 rounded-xl border-2 border-primary bg-primary/5 p-6 shadow-xl backdrop-blur-md">
              <div className="flex flex-col items-center justify-between gap-4 sm:flex-row">
                <div className="flex items-center gap-4">
                  <div className="flex h-12 w-12 items-center justify-center rounded-lg bg-primary text-white shadow-inner"><FileText className="h-6 w-6" /></div>
                  <div>
                    <h3 className="font-bold">{stagedFile.name}</h3>
                    <p className="text-xs text-muted-foreground">Ready for Cloud Archival • {(stagedFile.size / 1024 / 1024).toFixed(2)} MB</p>
                  </div>
                </div>
        ...
        <div className="rounded-xl border bg-card/80 backdrop-blur-md shadow-2xl">
          <div className="border-b p-5 flex justify-between items-center">
            ...
            <div className="flex items-center gap-2 rounded-md border bg-background px-3 py-1.5 w-64 shadow-sm">
              ...
              <input type="text" placeholder="Search archive..." className="bg-transparent text-sm outline-none w-full" />
            </div>
          </div>
                  ...
                  documents.map((doc) => (
                    <tr key={doc.id} className="hover:bg-primary/5 transition-colors group">
                      ...
                      <td className="px-6 py-4">
                        <Badge variant={statusConfig[doc.status]?.variant || "outline"}>{statusConfig[doc.status]?.label || doc.status}</Badge>
                      </td>
